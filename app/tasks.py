...
from app.db_models import db, Dataset, Chemical, QSARModel, Task, CVResults, Bioprofile, KDNNPrediction, Job
...
# Suppress sklearn and RDKit warnings
warnings.filterwarnings('ignore', category=UserWarning, module='sklearn')
warnings.filterwarnings('ignore', message='.*MorganGenerator.*')
warnings.filterwarnings('ignore', message='.*needs_proba.*')
...
def build_qsar(user_id, dataset_name, descriptors, algorithm, type, job_id=None, total_tasks=1):
    """Build a QSAR model as a background job."""
    job = get_current_job()

    # Only update job meta if running in RQ context
    if job:
...
def process_bioprofiler_job(job_id, file_path, session_dir):
    """
    Process Bioprofiler job in the background.
    Updates Job status as it progresses.

    Args:
        job_id: The unique job ID
        file_path: Path to the uploaded file with CIDs
        session_dir: Session directory for storing outputs
    """
    import signal
    ...
    logger.info(f"[{job_id}] ========== STARTING BIOPROFILER JOB ==========")
    logger.info(f"[{job_id}] File: {file_path}, Session: {session_dir}")

    # Get the job record
    job_record = Job.query.filter_by(job_id=job_id).first()
    if not job_record:
        logger.error(f"[{job_id}] ERROR: Job not found in database")
        return
    ...
    try:
        ...
        db.session.commit()
        logger.info(f"[{job_id}] Status updated to 'running'")

        # Step 1: Read compound IDs
        logger.info(f"[{job_id}] STEP 1: Reading compound IDs from file")
        target_compounds = []
        with open(file_path, "r") as csv_file:
            for line in csv_file:
                cid = line.strip()
                if cid:
                    target_compounds.append(cid)

        # Enforce CID limit
        if len(target_compounds) > 500:
            logger.error(f"[{job_id}] ERROR: Too many compounds ({len(target_compounds)} > 500 limit)")
            job_record.status = 'failed'
            job_record.updated_at = datetime.datetime.utcnow()
            db.session.commit()
            return

        logger.info(f"[{job_id}] Found {len(target_compounds)} compounds to process")

        # Step 2: Generate bioprofile (TIMEOUT: 600 seconds = 10 minutes)
        logger.info(f"[{job_id}] STEP 2: Generating bioprofile (timeout: 10 min)")
        bioprofile_outfile = os.path.join(session_dir, 'bioprofile_long.csv')
        try:
            run_with_timeout(bp.bioprofile, 600, target_compounds, chunk=100, outfile=bioprofile_outfile)
            logger.info(f"[{job_id}] ✓ Bioprofile generated successfully")
        except TimeoutError:
            raise Exception("Bioprofile generation timed out after 10 minutes")

        # Step 3: Generate matrix
        logger.info(f"[{job_id}] STEP 3: Creating matrix")
        bioprofile_matrix_outfile = os.path.join(session_dir, 'bioprofile_matrix.csv')
        bp.make_matrix(bioprofile_outfile, min_actives=1, outfile=bioprofile_matrix_outfile)
        logger.info(f"[{job_id}] ✓ Matrix created successfully")

        # Step 4: Generate heatmap
        logger.info(f"[{job_id}] STEP 4: Generating heatmap")
        heatmap_outfile = os.path.join(session_dir, 'heatmap.png')
        bp.get_heat_map(pd.read_csv(bioprofile_matrix_outfile, index_col=0), output_file=heatmap_outfile)
        logger.info(f"[{job_id}] ✓ Heatmap generated successfully")

        # Step 5: Select top assays
        logger.info(f"[{job_id}] STEP 5: Selecting top assays")
        selected_assays_file = os.path.join(session_dir, 'SelectedAssays.csv')
        bp.select_top_assays(bioprofile_matrix_outfile, selected_assays_file)
        logger.info(f"[{job_id}] ✓ Top assays selected successfully")

        # Step 6: Fetch and curate AID data (TIMEOUT: 300 seconds = 5 minutes)
        logger.info(f"[{job_id}] STEP 6: Fetching AID data (timeout: 5 min)")
        try:
            run_with_timeout(bp.fetch_and_save_aid_data, 300, selected_assays_file, session_dir=session_dir)
            logger.info(f"[{job_id}] ✓ AID data fetched and saved successfully")
        except TimeoutError:
            raise Exception("AID data fetch timed out after 5 minutes")

        # Step 7: Train models (optional - may fail with insufficient data)
        logger.info(f"[{job_id}] STEP 7: Training RF models (timeout: 5 min)")
        rf_metrics_outfile = os.path.join(session_dir, 'RF_metrics.csv')
        rf_training_succeeded = False
        try:
            run_with_timeout(bp.generate_all_rf_models_and_save_metrics, 300, output_file=rf_metrics_outfile, session_dir=session_dir)
            if os.path.exists(rf_metrics_outfile):
                rf_training_succeeded = True
                logger.info(f"[{job_id}] ✓ RF models generated successfully")

                # Plot metrics (only if training succeeded)
                logger.info(f"[{job_id}] STEP 7a: Plotting RF metrics")
                bp.plot_rf_metrics(metrics_file=rf_metrics_outfile, session_dir=session_dir)
                logger.info(f"[{job_id}] ✓ RF metrics plotted successfully")
            else:
                logger.info(f"[{job_id}] ⚠ RF training skipped - insufficient data")
        except TimeoutError:
            logger.warning(f"[{job_id}] ⚠ RF training timed out - continuing anyway")
        except Exception as rf_error:
            logger.warning(f"[{job_id}] ⚠ RF training failed: {rf_error} - continuing anyway")

        # Step 8: Filter AIDs (only if RF training succeeded)
        if rf_training_succeeded:
            logger.info(f"[{job_id}] STEP 8: Filtering selected AIDs")
            selected_matrix_outfile = os.path.join(session_dir, 'Selected_bioprofile_matrix.csv')
            bp.filter_selected_aids(matrix_file=bioprofile_matrix_outfile, output_file=selected_matrix_outfile, session_dir=session_dir)
            logger.info(f"[{job_id}] ✓ Filtered selected AIDs successfully")

            # Step 9: Replace 0s with predictions
            logger.info(f"[{job_id}] STEP 9: Filling predictions")
            filled_matrix_outfile = os.path.join(session_dir, 'DataFilled_bioprofile_matrix.csv')
            bp.replace_zeroes_with_predictions(matrix_file=selected_matrix_outfile, output_file=filled_matrix_outfile, session_dir=session_dir)
            logger.info(f"[{job_id}] ✓ Predictions filled successfully")
        else:
            logger.info(f"[{job_id}] STEP 8-9: Skipping RF-based filtering (no trained models)")
        ...
        job_record.updated_at = datetime.datetime.utcnow()
        db.session.commit()
        logger.error(f"[{job_id}] Job marked as failed in database")
        ...
        if os.path.exists(file_path):
            try:
                ...
                logger.warning(f"Job {job_id}: Failed to remove file {file_path}: {e}")
